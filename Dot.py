...
class Dot:
    ...
    def __init__(self):
        self.rect = pygame.Rect(
            Dot.startx, Dot.starty, Dot.w, Dot.h
        )  # This rectangle will be used to move the dot
    ...
    def update(self):
        if not self.dead and not self.reached and not self.hitWall:
            ...
            if self.isColliding():
                self.hitWall = True
            elif self.rect.colliderect(Dot.goal):
                self.reached = True
    ...
    def isColliding(self):
        ...
        if (
            self.rect.top < maze.margin_top + WALL_THICKNESS
            or self.rect.right > WIDTH - maze.margin_right - WALL_THICKNESS
            or self.rect.left < MAZE_BOT_OFFSET + maze.margin_left + WALL_THICKNESS
            or self.rect.bottom > maze.h + maze.margin_top - WALL_THICKNESS
        ):
            return True
        ...
        for j in range(self.rect.left, self.rect.right):
            if (
                maze.array[self.rect.top - maze.margin_top][
                    j - maze.margin_left - MAZE_BOT_OFFSET
                ]
                == 0
            ):
                return 1
            elif (
                maze.array[self.rect.bottom - maze.margin_top - 1][
                    j - maze.margin_left - MAZE_BOT_OFFSET
                ]
                == 0
            ):
        ...
        for j in range(self.rect.top, self.rect.bottom):
            if (
                maze.array[j - maze.margin_top][
                    self.rect.left - maze.margin_left - MAZE_BOT_OFFSET
                ]
                == 0
            ):
                return 1
                ...
                maze.array[j - maze.margin_top][
                    self.rect.right - 1 - maze.margin_left - MAZE_BOT_OFFSET
                ]
                == 0
            ):
                return 1
